...
load("./data/modified/local_view/lv_countyLevel_withNA.rdata")
...
for (y in unique(lv_countyLevel_withNA$transcript_year)) {
    ad <- lv_countyLevel_withNA %>%
...
#   and fema disaster declarations

allData_countyLevel_withNA <- left_join(lvacsru_algara, cvi,
                                        by = "stcounty_fips",
                                        relationship = "many-to-one"
) %>%
    left_join(fema_countyLevel) %>%
    relocate(census_division, census_region, transcript_year, has_transcript,
             .after = county_fips
    ) %>%
    relocate(ru_year, .after = edu_percentPop)

# save(allData_countyLevel_withNA, file = "./data/modified/all_data/allData_countyLevel_withNA.rdata")
